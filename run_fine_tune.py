...
def _parse_args():
    ...
    parser.add_argument('--dataset_path', type=str, default='../MELD.Preprocessed')
...
def get_optimizer_config(model, training_config, param_groups):
...
def main():
    ...
    optimizer_config = get_optimizer_config(model, training_config, param_groups)

    loss_fn = torch.nn.CrossEntropyLoss()

    # Training module
    training_module = ClassificatorModule.load_from_checkpoint("/logs/baseline/version_0/checkpoints/epoch=19-step=6260.ckpt",
                                                 model=model, loss_fn=loss_fn, optimizer_config=optimizer_config)  # that's the checkpoint after run.py
    # training_module = ClassificatorModule(model, loss_fn, optimizer_config)
    # training_module.eval()
    training_module.model.clip = training_module.model.clip.float()
